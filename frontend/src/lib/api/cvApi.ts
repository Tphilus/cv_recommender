...
export async function uploadCv(file: File): Promise<UploadResponse> {
  ...
  const { data } = await Axios.post<UploadResponse>("/cv/upload", formData);
  return data;
}

export async function getCandidate(candidateId: string): Promise<Candidate> {
  const { data } = await Axios.get<Candidate>(`/cv/${candidateId}`);
  return data;
}

export async function listCandidates(limit = 50): Promise<Candidate[]> {
  const { data } = await Axios.get<Candidate[]>("/cv", { params: { limit } });
  return data;
}

export async function getAnalysis(candidateId: string): Promise<AnalysisResponse> {
  const { data } = await Axios.get<AnalysisResponse>(`/cv/${candidateId}/analysis`);
  return data;
}

export async function getJobRecommendations(candidateId: string): Promise<JobMatchReport> {
  const { data } = await Axios.get<JobMatchReport>(`/jobs/recommendations/${candidateId}`);
  return data;
}

export async function deleteCandidate(candidateId: string): Promise<void> {
  await Axios.delete(`/cv/${candidateId}`);
}
...
export async function fetchCvPreviewBlob(candidateId: string): Promise<Blob> {
  ...
  const { data } = await Axios.get<Blob>(`/cv/${candidateId}/preview-file`, { responseType: "blob" });
  return data;
}
